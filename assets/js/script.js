...
function updateLanguage(lang) {
  ...
  // Bütün data-i18n elementlərini yeniləmək
  ...
  // Desktop dil düyməsini yeniləmək
  ...
  // HTML lang atributunu yeniləmək
...
/**
 * Dil seçimi hadisələrini qurmaq
 */
function setupLanguageHandlers() {
  // Desktop və mobil dropdown seçimləri
...
function setupScrollEffects() {
  ...
  window.addEventListener(
    ...
    debounce(function () {
      ...
      if (scrollToTop) {
        scrollToTop.classList.toggle("show", scrolled);
      }
    }, 100),
  );
...
/**
 * Kontakt formasının idarə edilməsi
 */
...
/**
 * Video avtomatik oynatma
 */
function setupVideoAutoplay() {
  const video = document.querySelector(".hero-video");
  if (video) {
    // Intersection Observer istifadə edərək
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            video.play().catch(() => {
              console.log("Autoplay bloklandı");
            });
          } else {
            video.pause();
          }
...
function setupLazyLoading() {
  if ("loading" in HTMLImageElement.prototype) {
    // Brauzer native lazy loading dəstəkləyir
    return;
  }

  // IntersectionObserver ilə manual lazy loading
